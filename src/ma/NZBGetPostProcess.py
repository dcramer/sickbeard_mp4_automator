...
try:
    ...
    print(
        "[ERROR] Wrong path to sickbeard_mp4_automator: "
...
if "NZBOP_SCRIPTDIR" in os.environ and not os.environ["NZBOP_VERSION"][0:5] < "11.0":
    ...
    if os.environ["NZBPP_UNPACKSTATUS"] == "0" and os.environ["NZBPP_PARSTATUS"] != "2":
        ...
        if (
            os.path.isfile(
                os.path.join(os.environ["NZBPP_DIRECTORY"], "_brokenlog.txt")
            )
            and not status == 1
        ):
            log.error(
    ...
    if not os.path.isdir(os.environ["NZBPP_DIRECTORY"]):
        log.error(
            "Post-Process: Nothing to post-process: destination directory ",
