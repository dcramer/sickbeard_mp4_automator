...
def process(dirName, settings, nzbName=None, status=0, logger=None):
    ...
    if status == 0:
        ...
        if rstate["success"]:
            ...
            log.error(
                "%s has NOT started on CouchPotatoServer for %s." % (command, nzbName1)
            )
        ...
        for index in range(len(movieid)):
            if movieid[index] == imdbid:
                ...
                log.info(
                    "Found movie id %s in database for release %s." % (movid, nzbName1)
                )
                n = n + 1
                break
        ...
        if n == 0:
            log.error(
                "Cound not find a movie in the database for release %s." % nzbName1
            )
            log.error(
        ...
        url = (
            ...
            + "/api/"
            + apikey
            + "/movie.searcher.try_next/?media_id="
            + movid
        )
        log.info("Opening URL: %s." % url)
        ...
        log.info(
            "Movie %s set to try the next best release on CouchPotatoServer." % movid
        )
        if delete_failed:
