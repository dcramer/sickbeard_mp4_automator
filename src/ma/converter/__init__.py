...
from .ffmpeg import FFMpeg, FFMpegError, FFMpegConvertError  # NOQA
...
class Converter(object):
    ...
    def parse_options(self, opt, twopass=None):
        ...
        for n in y:
            ...
            if "path" in x and "source" not in x:
                raise ConverterError(
                    "Cannot specify audio path without FFMPEG source number"
                )
    ...
    def thumbnail(
        self, fname, time, outfile, size=None, quality=FFMpeg.DEFAULT_JPEG_QUALITY
    ):
        """
