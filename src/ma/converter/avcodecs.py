...
class AudioCodec(BaseCodec):
    ...
    def parse_options(self, opt, stream=0):
        ...
        if "language" in safe:
            ...
            lang = (
                "und"
            )  # Never leave blank if not specified, always set to und for undefined
    ...
    def parse_options(self, opt, stream=0):
        ...
        if "default" in safe:
            optlist.extend(
                [
                    "-metadata:s:s:" + stream,
                    "disposition:default=" + str(safe["default"]),
                ]
            )
        if "forced" in safe:
            optlist.extend(
                ["-metadata:s:s:" + stream, "disposition:forced=" + str(safe["forced"])]
            )
        if "language" in safe:
            lang = str(safe["language"])
        else:
...
class VideoCopyCodec(BaseCodec):
    ...
    codec_name = "copy"
    encoder_options = {"map": int, "source": str}
...
class H264VAAPI(H264Codec):
    ...
    def _codec_specific_produce_ffmpeg_list(self, safe, stream=0):
        ...
        if "level" in safe:
            optlist.extend(
                ["-level", "%0.0f" % (safe["level"] * 10)]
            )  # Automatically multiplied by 10
        if "tune" in safe:
            optlist.extend(["-tune", safe["tune"]])
        # Start VF
