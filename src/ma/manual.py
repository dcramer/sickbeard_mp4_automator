...
from .extensions import tmdb_api_key, valid_tagging_extensions
...
settings = ReadSettings(os.path.dirname(__file__), "autoProcess.ini", logger=log)
...
def tmdbInfo(guessData):
    ...
    for movie in movies.iter_results():
        ...
        if foundname.lower() == origname.lower():
            print(
                "Matched movie title as: %s %s"
                % (
                    movie["title"].encode(sys.stdout.encoding, errors="ignore"),
                    movie["release_date"].encode(sys.stdout.encoding, errors="ignore"),
                )
            )
...
def tvdbInfo(guessData, tvdbid=None):
    ...
    try:
        print(
            "Matched TV episode as %s (TVDB ID:%d) S%02dE%02d"
            % (
...
def processFile(inputfile, tagdata, relativePath=None):
    ...
    if tagdata is False:
        ...
        tagmp4 = Tvdb_mp4(
            tvdbid, season, episode, language=settings.taglanguage, logger=log
        )
        try:
            print(
                "Processing %s Season %02d Episode %02d - %s"
                % (
    ...
    if MkvtoMp4(settings, logger=log).validSource(inputfile):
        ...
        if output:
            if (
                tagmp4 is not None
                and output["output_extension"] in valid_tagging_extensions
            ):
                try:
                    tagmp4.setHD(output["x"], output["y"])
                    tagmp4.writeTags(
                        output["output"], settings.artwork, settings.thumbnail
                    )
                except Exception as e:
                    print("There was an error tagging the file")
                    print(e)
            if (
                settings.relocate_moov
                and output["output_extension"] in valid_tagging_extensions
            ):
                converter.QTFS(output["output"])
            output_files = converter.replicate(
                output["output"], relativePath=relativePath
            )
...
def walkDir(dir, silent=False, preserveRelative=False, tvdbid=None, tag=True):
    for r, d, f in os.walk(dir):
        for file in f:
            ...
            relative = (
                os.path.split(os.path.relpath(filepath, dir))[0]
                if preserveRelative
                else None
            )
            try:
                if MkvtoMp4(settings, logger=log).validSource(filepath):
                    try:
                        print(
                            "Processing file %s"
                            % (filepath.encode(sys.stdout.encoding, errors="ignore"))
                        )
                    except Exception:
                        try:
                            print(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Manual conversion and tagging script for sickbeard_mp4_automator"
    )
    parser.add_argument(
        "-i",
        "--input",
    ...
    parser.add_argument(
        "-c", "--config", help="Specify an alternate configuration file location"
    )
    parser.add_argument(
        "-a",
        "--auto",
        action="store_true",
        help="Enable auto mode, the script will not prompt you for any further input, good for batch files. It will guess the metadata using guessit",
    )
    ...
    parser.add_argument(
        ...
        "--nomove",
        action="store_true",
        help="Overrides and disables the custom moving of file options that come from output_dir and move-to",
    )
    parser.add_argument(
        "-nc",
        "--nocopy",
        action="store_true",
        help="Overrides and disables the custom copying of file options that come from output_dir and move-to",
    )
    parser.add_argument(
        "-nd",
        "--nodelete",
    ...
    parser.add_argument(
        ...
        "--nopost",
        action="store_true",
        help="Overrides and disables the execution of additional post processing scripts",
    )
    parser.add_argument(
        "-pr",
        "--preserveRelative",
        action="store_true",
        help="Preserves relative directories when processing multiple files using the copy-to or move-to functionality",
    )
    parser.add_argument(
        "-cmp4",
        "--convertmp4",
        action="store_true",
        help="Overrides convert-mp4 setting in autoProcess.ini enabling the reprocessing of mp4 files",
    )
    parser.add_argument(
        "-m",
        "--moveto",
        help="Override move-to value setting in autoProcess.ini changing the final destination of the file",
    )
    ...
    if args["config"]:
        if os.path.exists(args["config"]):
            ...
            settings = ReadSettings(
                os.path.split(args["config"])[0],
                os.path.split(args["config"])[1],
                logger=log,
            )
        elif os.path.exists(os.path.join(os.path.dirname(__file__), args["config"])):
            print('Using configuration file "%s"' % (args["config"]))
            settings = ReadSettings(
                os.path.dirname(__file__), args["config"], logger=log
            )
        else:
            print(
