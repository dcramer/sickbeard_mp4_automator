...
class MkvtoMp4:
            ...
            if self.output_dir is not None:
                try:
                    outputfile = os.path.join(
                        self.output_dir, os.path.split(inputfile)[1]
                    )
    ...
    def validSource(self, inputfile):
        ...
        if (
            input_extension.lower() in valid_input_extensions
            or input_extension.lower() in valid_output_extensions
        ):
            ...
            self.log.debug(
                "%s is invalid with extension %s." % (inputfile, input_extension)
            )
            return False
    ...
    def needProcessing(self, inputfile):
        ...
        if (
            input_extension.lower() in valid_input_extensions
            or (
                self.processMP4 is True
                and input_extension.lower() in valid_output_extensions
            )
        ) and self.output_extension.lower() in valid_output_extensions:
    ...
    def estimateVideoBitrate(self, info):
        ...
        self.log.debug(
            "Estimated video bitrate is %s." % (total_bitrate - audio_bitrate)
        )
        return ((total_bitrate - audio_bitrate) / 1000) * 0.95
    ...
    def convert(self, inputfile, options, reportProgress=False):
        ...
        try:
            outputfile = os.path.join(
                output_dir.decode(sys.getfilesystemencoding()),
            ...
            outputfile = os.path.join(
                output_dir, filename + "." + self.output_extension
            )
        ...
        if os.path.abspath(inputfile) == os.path.abspath(outputfile):
            ...
            try:
                ...
                while os.path.isfile(outputfile):
                    outputfile = os.path.join(
                        output_dir,
                        filename + "(" + str(i) + ")." + self.output_extension,
                    )
                    i += i
                self.log.debug(
                    "Unable to rename inputfile. Setting output file name to %s."
                    % outputfile
                )
        ...
        conv = Converter(self.FFMPEG_PATH, self.FFPROBE_PATH).convert(
            ...
            options,
            timeout=None,
            preopts=options["preopts"],
            postopts=options["postopts"],
        )
    ...
    def QTFS(self, inputfile):
        ...
        if (
            self.parseFile(inputfile)[2] in valid_output_extensions
            and os.path.isfile(inputfile)
            and self.relocate_moov
        ):
    ...
    def replicate(self, inputfile, relativePath=None):
            ...
            for d in self.copyto:
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            shutil.copy(
                                inputfile.decode(sys.getfilesystemencoding()), d
                            )
                        except Exception:
                            shutil.copy(inputfile, d)
                        self.log.info("%s copied to %s." % (inputfile, d))
        ...
        if self.moveto:
            ...
            moveto = (
                os.path.join(self.moveto, relativePath) if relativePath else self.moveto
            )
