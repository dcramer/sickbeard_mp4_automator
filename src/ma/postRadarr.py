...
if MkvtoMp4(settings).validSource(inputfile):
    ...
    if output:
        ...
        if settings.tagfile and output["output_extension"] in valid_tagging_extensions:
            ...
            try:
                tagmp4 = tmdb_mp4(
                    imdbid, original=original, language=settings.taglanguage
                )
        ...
        try:
            ...
            if apikey != "":
                ...
                log.info(
                    "Requesting updated information from Radarr for movie ID %s."
                    % movieID
                )
                r = requests.get(url, headers=headers)
                payload = r.json()
                ...
                log.info(
                    "Radarr monitoring information updated for movie %s."
                    % success["title"]
                )
            else:
                log.error(
                    "Your Radarr API Key can not be blank. Update autoProcess.ini."
                )
        except:
