...
if MkvtoMp4(settings).validSource(inputfile):
    ...
    if output:
        ...
        if settings.tagfile and output["output_extension"] in valid_tagging_extensions:
            log.info(
                "Tagging %s with ID %s season %s episode %s."
                % (inputfile, tvdb_id, season, episode)
            )
            try:
                tagmp4 = Tvdb_mp4(
                    tvdb_id, season, episode, original, language=settings.taglanguage
                )
        ...
        try:
            ...
            if apikey != "":
                ...
                log.info(
                    "Requesting episode information from Sonarr for series ID %s."
                    % seriesID
                )
                ...
                attempts = 0
                while (
                    command["state"].lower() not in ["complete", "completed"]
                    and attempts < 6
                ):
                ...
                log.info(
                    "Requesting updated episode information from Sonarr for series ID %s."
                    % seriesID
                )
                r = requests.get(url, headers=headers)
                payload = r.json()
                ...
                for ep in payload:
                    if (
                        int(ep["episodeNumber"]) == episode
                        and int(ep["seasonNumber"]) == season
                    ):
                        sonarrepinfo = ep
                        break
