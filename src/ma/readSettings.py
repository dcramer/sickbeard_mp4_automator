...
class ReadSettings:
    def __init__(self, directory, filename, logger=None):
        ...
        write = (
            False
        )  # Will be changed to true if a value is missing from the config file and needs to be written
        ...
        section = "MP4"
        self.ffmpeg = os.path.normpath(
            self.raw(config.get(section, "ffmpeg"))
        )  # Location of FFMPEG.exe
        self.ffprobe = os.path.normpath(
            self.raw(config.get(section, "ffprobe"))
        )  # Location of FFPROBE.exe
        ...
        if self.output_dir == "":
            ...
            self.output_dir = os.path.normpath(
                self.raw(self.output_dir)
            )  # Output directory
        self.copyto = config.get(
            section, "copy_to"
        )  # Directories to make copies of the final product
        if self.copyto == "":
            self.copyto = None
        else:
        ...
        self.moveto = config.get(
            section, "move_to"
        )  # Directory to move final product to
        if self.moveto == "":
            self.moveto = None
        else:
        ...
        self.delete = config.getboolean(
            section, "delete_original"
        )  # Delete original file
        self.relocate_moov = config.getboolean(
            section, "relocate_moov"
        )  # Relocate MOOV atom to start of file
        ...
        self.acodec = config.get(
            section, "audio-codec"
        ).lower()  # Gets the desired audio codec, if no valid codec selected, default to AC3
        if self.acodec == "":
            self.acodec == ["ac3"]
        else:
        ...
        self.audio_copyoriginal = config.getboolean(
            section, "audio-copy-original"
        )  # Copies the original audio track regardless of format if a converted track is being generated

        self.afilter = (
            config.get(section, "audio-filter").lower().strip()
        )  # Audio filter
        if self.afilter == "":
            self.afilter = None

        self.audio_first_language_track = config.getboolean(
            section, "audio-first-track-of-language"
        )  # Only take the first audio track in a whitelisted language, then no more
        ...
        self.iOSLast = config.getboolean(
            section, "ios-move-last"
        )  # Moves the iOS audio track to the last in the series of tracks

        self.iOSfilter = (
            config.get(section, "ios-audio-filter").lower().strip()
        )  # iOS audio filter
        if self.iOSfilter == "":
            self.iOSfilter = None
        ...
        self.embedonlyinternalsubs = config.getboolean(
            section, "embed-only-internal-subs"
        )
        ...
        self.hevc_qsv_decoder = config.getboolean(
            section, "use-hevc-qsv-decoder"
        )  # only supported on 6th gen intel and up.
        ...
        self.awl = (
            config.get(section, "audio-language").strip().lower()
        ...
        self.swl = (
            config.get(section, "subtitle-language").strip().lower()
        ...
        self.fullpathguess = config.getboolean(
            section, "fullpathguess"
        )  # Guess using the full path or not
        self.tagfile = config.getboolean(section, "tagfile")  # Tag files with metadata
        self.taglanguage = (
            config.get(section, "tag-language").strip().lower()
        )  # Language to tag files
        ...
        self.artwork = config.get(
            section, "download-artwork"
        ).lower()  # Download and embed artwork
        ...
        if self.uTorrent["output_dir"] == "":
            ...
            self.uTorrent["output_dir"] = os.path.normpath(
                self.raw(self.uTorrent["output_dir"])
            )  # Output directory
        ...
        if self.deluge["output_dir"] == "":
            ...
            self.deluge["output_dir"] = os.path.normpath(
                self.raw(self.deluge["output_dir"])
            )  # Output directory
        ...
        if self.SAB["output_dir"] == "":
            ...
            self.SAB["output_dir"] = os.path.normpath(
                self.raw(self.SAB["output_dir"])
            )  # Output directory
