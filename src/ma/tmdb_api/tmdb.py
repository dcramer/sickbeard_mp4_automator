...
def configure(api_key, language="en"):
    ...
    config["urls"]["movie.search"] = (
        "https://api.themoviedb.org/3/search/movie?query=%%s&api_key=%(apikey)s&page=%%s"
        % (config)
    )
    config["urls"][
        "movie.info"
    ] = "https://api.themoviedb.org/3/movie/%%s?api_key=%(apikey)s" % (config)
    config["urls"]["people.search"] = (
        "https://api.themoviedb.org/3/search/person?query=%%s&api_key=%(apikey)s&page=%%s"
        % (config)
    )
    config["urls"][
        "collection.info"
    ] = "https://api.themoviedb.org/3/collection/%%s?api_key=%(apikey)s" % (config)
    config["urls"]["movie.alternativetitles"] = (
        "https://api.themoviedb.org/3/movie/%%s/alternative_titles?api_key=%(apikey)s"
        % (config)
    )
    config["urls"][
        "movie.casts"
    ] = "https://api.themoviedb.org/3/movie/%%s/casts?api_key=%(apikey)s" % (config)
    config["urls"][
        "movie.images"
    ] = "https://api.themoviedb.org/3/movie/%%s/images?api_key=%(apikey)s" % (config)
    config["urls"][
        "movie.keywords"
    ] = "https://api.themoviedb.org/3/movie/%%s/keywords?api_key=%(apikey)s" % (config)
    config["urls"][
        "movie.releases"
    ] = "https://api.themoviedb.org/3/movie/%%s/releases?api_key=%(apikey)s" % (config)
    config["urls"][
        "movie.trailers"
    ] = "https://api.themoviedb.org/3/movie/%%s/trailers?api_key=%(apikey)s" % (config)
    config["urls"]["movie.translations"] = (
        "https://api.themoviedb.org/3/movie/%%s/translations?api_key=%(apikey)s"
        % (config)
    )
    config["urls"]["person.info"] = (
        "https://api.themoviedb.org/3/person/%%s?api_key=%(apikey)s&append_to_response=images,credits"
        % (config)
    )
    config["urls"][
        "latestmovie"
    ] = "https://api.themoviedb.org/3/latest/movie?api_key=%(apikey)s" % (config)
    config["urls"][
        "config"
    ] = "https://api.themoviedb.org/3/configuration?api_key=%(apikey)s" % (config)
    config["urls"]["request.token"] = (
        "https://api.themoviedb.org/3/authentication/token/new?api_key=%(apikey)s"
        % (config)
    )
    config["urls"]["session.id"] = (
        "https://api.themoviedb.org/3/authentication/session/new?api_key=%(apikey)s&request_token=%%s"
        % (config)
    )
    config["urls"]["movie.add.rating"] = (
        "https://api.themoviedb.org/3/movie/%%s/rating?session_id=%%s&api_key=%(apikey)s"
        % (config)
    )
    config["api"] = {}
            ...
            if int(pages) > 1:  #
                for i in range(2, int(pages) + 1):  #  Thanks @tBuLi
                    self.movies["results"].extend(
                        self.getJSON(
                            config["urls"]["movie.search"] % (title, str(i)),
                            language=language,
                        )["results"]
                    )
...
class Movie(Core):
    def __init__(self, movie_id, language=None):
        ...
        self.casts = self.getJSON(
            config["urls"]["movie.casts"] % self.movie_id, language=language
        )
        self.releases = self.getJSON(
            config["urls"]["movie.releases"] % self.movie_id, language=language
        )
    ...
    def get_trailers(self, language=None):
        return self.getJSON(
            config["urls"]["movie.trailers"] % self.movie_id, language=language
        )
            ...
            if int(pages) > 1:
                for i in range(2, int(pages) + 1):
                    self.people["results"].extend(
                        self.getJSON(
                            config["urls"]["people.search"] % (people_name, str(i)),
                            language=language,
                        )["results"]
                    )
