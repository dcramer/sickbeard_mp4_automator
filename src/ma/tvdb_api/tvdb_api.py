...
class Season(dict):
    ...
    def __getitem__(self, episode_number):
        if episode_number not in self:
            raise tvdb_episodenotfound(
                "Could not find episode %s" % (repr(episode_number))
            )
        else:
...
class Episode(dict):
    ...
    def search(self, term=None, key=None):
        ...
        for cur_key, cur_value in self.items():
            cur_key, cur_value = (
                text_type(cur_key).lower(),
                text_type(cur_value).lower(),
            )
    ...
    def __init__(
        ...
        if not IS_PY2:  # FIXME: Allow using requests in Python 2?
            ...
            if cache is True:
                ...
                )  # default opener with no caching
                ...
                raise ValueError(
                    "Invalid value for Cache %r (type was %s)" % (cache, type(cache))
                )
        ...
        if self.config["search_all_languages"]:
            self.config["url_getSeries"] = (
                u"%(base_url)s/api/GetSeries.php?seriesname=%%s&language=all"
                % self.config
            )
        else:
            self.config["url_getSeries"] = (
                u"%(base_url)s/api/GetSeries.php?seriesname=%%s&language=%(language)s"
                % self.config
            )

        self.config["url_epInfo"] = (
            u"%(base_url)s/api/%(apikey)s/series/%%s/all/%%s.xml" % self.config
        )
        self.config["url_epInfo_zip"] = (
            u"%(base_url)s/api/%(apikey)s/series/%%s/all/%%s.zip" % self.config
        )

        self.config["url_seriesInfo"] = (
            u"%(base_url)s/api/%(apikey)s/series/%%s/%%s.xml" % self.config
        )
        self.config["url_actorsInfo"] = (
            u"%(base_url)s/api/%(apikey)s/series/%%s/actors.xml" % self.config
        )

        self.config["url_seriesBanner"] = (
            u"%(base_url)s/api/%(apikey)s/series/%%s/banners.xml" % self.config
        )
        ...
        if not IS_PY2:
            ...
            try:
                ...
                if "x-local-cache" in resp.headers:
                    log().debug(
                        "URL %s was cached in %s" % (url, resp.headers["x-local-cache"])
                    )
                    if recache:
            ...
            if "gzip" in resp.headers.get("Content-Encoding", ""):
                ...
                raise tvdb_error(
                    "Received gzip data from thetvdb.com, but could not correctly handle it"
                )
    ...
    def _getShowData(self, sid, language):
        ...
        for cur_ep in epsEt.findall("Episode"):
            ...
            if self.config["dvdorder"]:
                ...
                use_dvd = (
                    cur_ep.find("DVD_season").text != None
                    and cur_ep.find("DVD_episodenumber").text != None
                )
            else:
                use_dvd = False
            ...
            if use_dvd:
                elem_seasnum, elem_epno = (
                    cur_ep.find("DVD_season"),
                    cur_ep.find("DVD_episodenumber"),
                )
            else:
                elem_seasnum, elem_epno = (
                    cur_ep.find("SeasonNumber"),
                    cur_ep.find("EpisodeNumber"),
                )
            ...
            if elem_seasnum is None or elem_epno is None:
                ...
                log().debug(
                    " ".join(
                        "%r is %r" % (child.tag, child.text)
                        for child in cur_ep.getchildren()
                    )
                )
