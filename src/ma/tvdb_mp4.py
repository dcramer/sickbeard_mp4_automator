...
class Tvdb_mp4:
    def __init__(
        self, show, season, episode, original=None, language="en", logger=None
    ):
    ...
    def writeTags(self, mp4Path, artwork=True, thumbnail=False):
        ...
        video["\xa9alb"] = (
            self.show + ", Season " + str(self.season)
        )  # iTunes Album as Season
        video["tves"] = [self.episode]  # Episode number
        video["trkn"] = [
            (int(self.episode), len(self.seasondata))
        ...
        ] = self.setRating()  # iTunes content rating
    ...
    def getArtwork(self, mp4Path, filename="cover", thumbnail=False):
        ...
        if poster is None:
            if thumbnail:
                try:
                    poster = urlretrieve(
                        self.episodedata["filename"],
                        os.path.join(
                            tempfile.gettempdir(), "poster-%s.jpg" % self.title
                        ),
                    )[0]
                except Exception as e:
                ...
                try:
                    for bannerid in self.showdata["_banners"]["season"][
                        "season"
                    ].keys():
                        if str(
                            self.showdata["_banners"]["season"]["season"][bannerid][
                                "season"
                            ]
                        ) == str(self.season):
                            poster = Poster()
                            poster.ratingcount = int(
                                self.showdata["_banners"]["season"]["season"][bannerid][
                                    "ratingcount"
                                ]
                            )
                            if poster.ratingcount > 0:
                                poster.rating = float(
                                    self.showdata["_banners"]["season"]["season"][
                                        bannerid
                                    ]["rating"]
                                )
                            poster.bannerpath = self.showdata["_banners"]["season"][
                                "season"
                            ][bannerid]["_bannerpath"]
...
def main():
    if len(sys.argv) > 4:
        ...
        tvdb_mp4_instance = Tvdb_mp4(
            show=tvdb_id, season=tvdb_season, episode=tvdb_episode
        )
