...
from mutagen._util import cdata, insert_bytes, DictProxy, MutagenError, hashable, enum
    ...
    def __render_8int(self, key, value):
        try:
            ...
            if min(value) < 0 or max(value) >= 2 ** 8:
                raise MP4MetadataValueError("invalid 8 bit integers: %r" % value)
        except TypeError:
            raise MP4MetadataValueError("%s must be a list of 8 bit integers" % (key))
    ...
    def __render_32int(self, key, value):
        try:
            ...
            if min(value) < 0 or max(value) >= 2 ** 32:
                raise MP4MetadataValueError("invalid 32 bit integers: %r" % value)
        except TypeError:
            raise MP4MetadataValueError("%s must be a list of 32 bit integers" % (key))
        ...
        for key, value in iteritems(self):
            ...
            if key == "covr":
                values.append(
                    ...
                    % (
                        key,
                        ", ".join(["[%d bytes of data]" % len(data) for data in value]),
                    )
                )
